...
const GAS_URL = "https://script.google.com/macros/s/AKfycbyhDMYON1CVS2KEx0tXl6D7emDQDkWK-9XhrVvj4-VzwZw7f1tPKTsVaVCpbQCBxa-D/exec";
...
function renderTable() {
    ...
    applications.forEach((app, index) => {
        ...
        // Кнопка статуса меняет цвет и текст в зависимости от состояния
        const statusBtnColor = app.completed ? "#10b981" : "#9ca3af";
        const statusBtnText = app.completed ? "Выполнено" : "Не вып.";
        ...
        tr.innerHTML = `
            ...
            <td style="text-align: center;">
                <button onclick="toggleStatus(${index})" style="background-color: ${statusBtnColor}; color: white; border: none; border-radius: 6px; cursor: pointer; padding: 6px 12px; font-size: 0.75rem; transition: 0.2s;">
                    ${statusBtnText}
                </button>
            </td>
...
window.toggleStatus = function(index) {
    ...
    // Мгновенно перерисовываем таблицу (чтобы кнопка стала зеленой)
    renderTable();

    // Отправляем команду на обновление статуса в Google Таблицы
    if (GAS_URL && !GAS_URL.includes("ВСТАВЬТЕ_URL")) {
        fetch(GAS_URL, {
            method: 'POST',
            body: JSON.stringify({
                action: 'updateStatus', // Специальная метка для гугла
                id: applications[index].id,
                completed: applications[index].completed
            }),
            headers: {
                'Content-Type': 'text/plain;charset=utf-8',
            }
...
function downloadWord(app) {
    ...
    const htmlContent = `
        ...
        <div class="WordSection1">
            ...
            <p>Исходя из условий эксплуатации и представленной документации, на основании заявки заказчика ${app.fio} мной, заместителем главного инженера ООО "Облбытгаз" Филимоновым Алексеем Викторовичем после проведения контроля газового оборудования проведено его техническое диагностирование:</p>
            <p>наименование оборудования: ${app.equipment} __________________</p>
